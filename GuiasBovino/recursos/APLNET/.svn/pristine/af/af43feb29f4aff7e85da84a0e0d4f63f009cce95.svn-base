...
$.widget("ui.vcw", {
	...
	_StringToXMLDom: function(string){
		var xmlDoc=null;
		if (window.DOMParser)
		{
		...
		if(xmlDoc.getElementsByTagName("parsererror").length==1 || xmlDoc.getElementsByTagName("ConfiguracionVisor").length==0)xmlDoc=null;
    ...
    _setOptions: function(options) {
		...
		try{
			vconf = self._StringToXMLDom(options["confFile"]);
		}catch(ex){vconf=""}
		if (vconf==""  || vconf==null){
		  try{
				vconf = self._StringToXMLDom(textoConfVNormal);
		  }catch(ex){vconf=""}
		}
		if (vconf=="" || vconf==null){
			self._getConfiguracion();
		}else{
			self._objConfiguracion=vconf;
    ...
    _init: function() {
		...
		if (vconf==""  || vconf==null){
		  try{
				vconf = self._StringToXMLDom(textoConf);
		  }catch(ex){vconf=""}
		}
		debugger;
		if (vconf=="" || vconf==null){
		...
		////31102017 pendiente revisar por David
		if(control.length>0){
			control[0].events.on({
				"activate": function() {
    ...
    _getConfiguracion: function() {
		...
		contentType: "application/json; charset=utf-8",
		data: null,
		url:self.options.confFile ,
		complete: function(jsondata, stat) {
				debugger;
                self._objConfiguracion = jsondata.responseText;
				var vconf=jsondata.responseText;
                if ($.isFunction(self.options.confCallback)) {
                    vconf = self.options.confCallback.call(this, self._objConfiguracion);
                }
				 self._objConfiguracion=self._StringToXMLDom(vconf);
		},
		fail:function(err){
				alert(err)
			}
